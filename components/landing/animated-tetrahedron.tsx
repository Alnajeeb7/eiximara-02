...
export function AnimatedTetrahedron() {
  ...
  useEffect(() => {
    ...
    // Color gradient from purple to pink to teal
    const getColor = (depth: number, faceIndex: number) => {
      const colors = [
        { r: 127, g: 90, b: 240 },   // Purple
        { r: 255, g: 78, b: 205 },   // Pink
        { r: 44, g: 182, b: 125 },   // Teal
      ];
      const colorIndex = faceIndex % colors.length;
    ...
    const render = () => {
      ...
      const points: { x: number; y: number; z: number; char: string; colorIndex: number }[] = [];
      ...
      points.forEach((point) => {
        ...
        const color = getColor(depth, point.colorIndex);
